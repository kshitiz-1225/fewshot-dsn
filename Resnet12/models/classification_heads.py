...
def ProtoNetHead(query, support, support_labels, n_way, n_shot, normalize=True):
    ...
    BB = (prototypes * prototypes).sum(dim=2,
                                       keepdim=True).reshape(tasks_per_batch, 1, n_way)
...
def SubspaceNetHead(query, support, support_labels, n_way, n_shot, normalize=True):
    ...
    tasks_per_batch = query.size(0)
    n_support = support.size(1)  # [4,25,2560]
    n_query = query.size(1)  # [4,25,2560]
    d = query.size(2)
    # support_labels [4,25]
    ...
    support_labels_one_hot = one_hot(
        support_labels.view(tasks_per_batch * n_support), n_way)  # [4,25] [100] --> [100,5]
    # support_labels_one_hot = support_labels_one_hot.view(tasks_per_batch, n_support, n_way)

    support_reshape = support.view(
        tasks_per_batch * n_support, -1)  # [100,2560]

    support_labels_reshaped = support_labels.contiguous().view(-1)  # [100]

    class_representatives = []
    for nn in range(n_way):
        idxss = (support_labels_reshaped == nn).nonzero()  # [20,1]
        all_support_perclass = support_reshape[idxss, :]  # [20,1,2560]

        class_representatives.append(
            all_support_perclass.view(tasks_per_batch, n_shot, -1))  # [4,5,2560]

    class_representatives = torch.stack(class_representatives)  # [5,4,5,2560]

    class_representatives = class_representatives.transpose(
        0, 1)  # tasks_per_batch, n_way, n_support, -1

    class_representatives = class_representatives.transpose(
        2, 3).contiguous().view(tasks_per_batch*n_way, -1, n_shot)  # [20,2560,5]
    ...
    for cc in range(tasks_per_batch*n_way):
        batch_idx = cc//n_way
        qq = query[batch_idx]  # [25,2560]
        uu, _, _ = torch.svd(class_representatives[cc].double())
        uu = uu.float()  # [2560,5]
        subspace = uu[:, :n_shot-1].transpose(0, 1)  # [4,2560]
        projection = subspace.transpose(0, 1).mm(
            subspace.mm(qq.transpose(0, 1))).transpose(0, 1)  # [25,2560]
        dist_perclass = torch.sum((qq - projection)**2, dim=-1)  # [25]
        dist.append(dist_perclass)

    ##############################################################################

    dist = torch.stack(dist).view(
        tasks_per_batch, n_way, -1).transpose(1, 2)  # [4,25,5]
