    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        for i, batch in enumerate(tqdm(dloader_train(epoch)), 1):
            ...
            logit_query = cls_head(
                emb_query, emb_support, labels_support, opt.train_way, opt.train_shot)  # [4, 25, 5]

            smoothed_one_hot = one_hot(labels_query.reshape(-1), opt.train_way)
            smoothed_one_hot = smoothed_one_hot * \
                (1 - opt.eps) + (1 - smoothed_one_hot) * \
                opt.eps / (opt.train_way - 1)
        ...
        for i, batch in enumerate(tqdm(dloader_val(epoch)), 1):
            ...
            logit_query = cls_head(
                emb_query, emb_support, labels_support, opt.test_way, opt.val_shot)
